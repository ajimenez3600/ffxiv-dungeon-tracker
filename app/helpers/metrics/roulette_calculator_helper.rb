module Metrics::RouletteCalculatorHelper
  ...
  def get_xp_for_roulette(roulette, start_level)
    entries = InstanceEntry.where(start_level: start_level, roulette: roulette, xp_outlier: false).select { |x| x.patch_number >= Rails.application.config.min_patch_number }
